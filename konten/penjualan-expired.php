...
<div class="content-wrapper">
  ...
  <div class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
          <h1 class="m-0">Pencatatan Barang Expired</h1>
        </div><!-- /.col -->
        <div class="col-sm-6">
          <ol class="breadcrumb float-sm-right">
            <li class="breadcrumb-item"><a href="#">Toko</a></li>
            <li class="breadcrumb-item"><a href="index.php?p=daftar-penjualan">Stok Expired</a></li>
            ...
            <div class="card-body">
              ...
              <table class="table table-bordered table-striped" style="width:100%;">
                ...
                <tbody>
                  ...
                  $sql2 = "SELECT keranjang.*,produk.nama,produk.qty from keranjang,produk where keranjang.id_produk=produk.id_produk and id_user=$id_user and jenis!='penjualan'";
                  $query2 = mysqli_query($koneksi, $sql2);
                  $no = 0;
                  $grandtotal = 0;
                  while ($kolom2 = mysqli_fetch_array($query2)) {
                    $no++;
                    $hpp = number_format($kolom2['hpp']);
                    $jumlah = number_format($kolom2['jumlah']);
                    $subtotal = number_format($kolom2['jumlah'] * $kolom2['hpp']);
                    $grandtotal = $grandtotal + ($kolom2['jumlah'] * $kolom2['hpp']);
                    $token = md5($kolom2['id_keranjang']);
                    echo "
                    <tr>
                      <td><a href='aksi/penjualan_expired.php?aksi=keranjang-hapus&token=$token'><i class='fas fa-trash'></a></i></td>
                      <td>$no</td>
                      <td>$kolom2[nama]</td>
                      <td align=right>$hpp</td>
                      <td align=right style='width:150px;'>                      
                        <form action='aksi/penjualan_expired.php' method='post'>
                        <input type='hidden' name='aksi' value='keranjang-ubah'> 
                        <input type='hidden' name='id' value='$kolom2[id_keranjang]'> 
                        <div class='form-row'>
                              <div class='col'>
                                  <input type='number' name='qty' class='form-control form-control-sm mb-2' min='1' max='$kolom2[qty]' value='$kolom2[jumlah]'>
                              </div>
                              <div class='col'>
                ...
                <tfoot>
                  ...
                  <br><i><strong>Note : Transaksi akan tercatat sebagai penjualan dengan nilai belanja Rp. 0</strong></i>
            </div>
          </div>
          <?php
          if ($grandtotal > 0) {
            ...
            echo "<button type='button' class='btn btn-success btn-block' data-toggle='modal' data-target='#simpanJualModalKas'><i class='fas fa-save'></i> Simpan Transaksi </button>";
            echo "</div>";
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <table id="finditem" class="table table-bordered table-striped" style="width:100%;">
          ...
          <!-- Isi Tabel -->
          <?php
          $sql = "SELECT produk.*,produk_kategori from produk,produk_kategori where produk.id_produk_kategori=produk_kategori.id_produk_kategori AND qty>=1 order by nama";
          ...
          ?>
            <tr>
              ...
              <td><?= $kolom['qty']; ?></td>
              <td>
                <button type="button" class="btn btn-link"><a href="aksi/penjualan_expired.php?aksi=keranjang-tambah&token=<?= md5($kolom['id_produk']); ?>"><i class="fas fa-check"></i></a></button>
              </td>
            </tr>
...
<div class="modal fade" id="simpanJualModalKas" tabindex="-1" aria-labelledby="simpanJualModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="simpanJualModalLabel">Simpan Transaksi Penghapusan Stok Expired</h5>
        <button type="button" class="btn-close" data-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        <form action="aksi/penjualan_expired.php" method="post">
          ...
          <input type="hidden" id="kupon_belanja" name="kupon_belanja" value="0">
          ...
          <div class="form-row">
            <div class="form-group col-sm-6">
              ...
              <select name="id_anggota" id="id_anggota1" class="select2bs4 form-control" required>                
                <?php
                $sql1 = "select * from anggota WHERE id_anggota=1 order by nama";
          ...
          <table class="table">
            <tr>
              <td>Total Nilai Stok Expired</td>
              <td>:</td>
